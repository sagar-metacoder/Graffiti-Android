package com.app.graffiti.distributor
...
/**
 * [DistributorDashboardActivity] : <p>
 *
 * </p>
...
class DistributorDashboardActivity : BaseActivity(),
        NetworkCallBack<Response<ResponseBody>>,
        RecyclerAdapter.ItemClickListener {
    companion object {
        val TAG = DistributorDashboardActivity::class.java.simpleName
                ?: "DistributorDashboardActivity"
    }
    ...
    private lateinit var distributorTargetHandler: WebServiceProvider.UserTarget
    private lateinit var distributorTargetCall: Network
    private lateinit var distributorTargetAdapter: UserTargetPagerAdapter
    ...
    override fun onCreateOptionsMenu(menu: Menu?): Boolean {
        menuInflater.inflate(R.menu.distributor_dashboard_menu, menu)
        return true
    }

    override fun onOptionsItemSelected(item: MenuItem?): Boolean {
        return when (item?.itemId) {
            R.id.distributor_dashboard_logout -> {
            ...
            if (bundle.getString(Common.EXTRA_DASHBOARD_ITEM, "") != "") {
                when (bundle.getString(Common.EXTRA_DASHBOARD_ITEM)) {
                    getString(R.string.item_dealer) -> {
                        startActivity(
                                Intent(this, DistributorDealerListActivity::class.java)
    ...
    private fun setUpAppBar() {
        ...
        activity_dashboard_appBar_bgContainer?.setBackgroundResource(R.drawable.distributor_appbar_bg)
...
//        list.add(getString(R.string.item_dealer_report))
        return list
    }

    private fun getDistributorTarget() {
        distributorTargetHandler = WebServiceProvider.UserTarget(
                user?.userId ?: -1,
                Common.UserType.DISTRIBUTOR.userType
        )
        distributorTargetCall = Network
                .Builder(
                        this,
                        distributorTargetHandler
                )
                .setFileExtension(".json")
                .build()
        distributorTargetCall.call(true, this)
    }

    private fun setDistributorTarget() {
        distributorTargetAdapter = UserTargetPagerAdapter(this, targetList)
        activity_dashboard_viewPager_userInfo?.adapter = distributorTargetAdapter
