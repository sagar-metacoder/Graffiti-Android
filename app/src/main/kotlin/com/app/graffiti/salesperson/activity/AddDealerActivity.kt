...
class AddDealerActivity : BaseActivity(), NetworkCallBack<Response<ResponseBody>> {
    companion object {
        val TAG = AddDealerActivity::class.java.simpleName ?: "AddDealerActivity"
    }
    ...
    private lateinit var addDealerCall: Network
    private lateinit var addDealer: WebServiceProvider.AddDealer
    ...
    private fun setViewAsPerUser() {
        when (userType) {
            Common.UserType.SALES_PERSON.userType -> {
                activity_add_distributor_dealer_spinner_chooseDistributor?.visibility = View.VISIBLE
                getDistributorList()
            }
            else -> {
    ...
    private fun addUser() {
        addDealer = WebServiceProvider.AddDealer(
                if (userType == Common.UserType.SALES_PERSON.userType)
        ...
        addDealerCall = Network.Builder(this, addDealer)
                .build()
        addDealerCall.call(this)
    }
    ...
    private fun setSpinnerDataAndListeners() {
        ...
        activity_add_distributor_dealer_spinner_chooseDistributor?.adapter = object : ArrayAdapter<String>(this, android.R.layout.simple_spinner_dropdown_item, distributorsName) {
            override fun getCount(): Int {
                val count = super.getCount()
                return if (count > 1) count - 1 else count
            }
        }
        activity_add_distributor_dealer_spinner_chooseDistributor?.setSelection(
                activity_add_distributor_dealer_spinner_chooseDistributor?.adapter?.count
                        ?: 0
        )
        activity_add_distributor_dealer_spinner_chooseDistributor?.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
    ...
    override fun onCachedResponse(response: String?, apiHandler: ApiHandler?) {
        when (apiHandler?.flag?.requestFlag) {
            WebServiceProvider.Flag.ADD_DEALER.flag -> {
                Logger.log(TAG, "onCachedResponse : Flag ${apiHandler.flag.requestFlagName}, Response $response", Logger.Level.WARN)
            }
            WebServiceProvider.Flag.DISTRIBUTORS_OF_SALES_PERSON.flag -> {
                activity_add_distributor_dealer_spinner_chooseDistributor?.visibility = View.GONE
                try {
    ...
    override fun onRequestComplete(response: Response<ResponseBody>?, apiHandler: ApiHandler?) {
        when (apiHandler?.flag?.requestFlag) {
            ...
            WebServiceProvider.Flag.DISTRIBUTORS_OF_SALES_PERSON.flag -> {
                ...
                Logger.log(TAG, "onRequestComplete : Invalid Flag found ${apiHandler?.flag?.requestFlagName}, Response $response", Logger.Level.ERROR)
            }
        }
    }
}